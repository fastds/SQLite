...
struct Pager{
  sqlite3_vfs *pVfs;          /* 用于IO操作的OS功能 */		
  u8 exclusiveMode;           /* 布尔值，用于指示是否为互斥模式 */
  u8 journalMode;             /* 日志模式：取值为PAGER_JOURNALMODE_* */			
  u8 useJournal;              /* 指示是否使用回滚日志 */
  u8 noSync;                  /* 指示是否同步日志 */
  u8 fullSync;                /* 指示做额外的同步操作以获得更好的健壮性 */
  u8 ckptSyncFlags;           /* 检查点的同步标记：取值为SYNC_NORMAL/SYNC_FULL */			
  u8 walSyncFlags;            /* wal写操作的同步标记：取值为SYNC_NORMAL/SYNC_FULL */		
  u8 syncFlags;               /* 其他情况的同步标记SYNC_NORMAL or SYNC_FULL */			
  u8 tempFile;                /* 指示数据库文件是否为临时文件 */				
  u8 readOnly;                /* 指示数据库的只读属性 */
  u8 memDb;                   /* 指示是否为内存数据库，若为true，则没有对于文件的磁盘I/O */
  /**************************************************************************
  ** 以下为描述Pager状态的成员变量，它们在函数操作期间不断被改变
  */
  ...
  /*
  ** 以下是与pager配置相关的变量，这些变量第一次被创建后很少被改变，除非重要的模式信息被改变时才改变
  如page_size，locking_mode, journal_mode等信息发生改变） **/
...
static const unsigned char aJournalMagic[] = {
  0xd9, 0xd5, 0x05, 0xf9, 0x20, 0xa1, 0x63, 0xd7,								//仅用于完整性检查（八个字节）
};
...
** of this.
给数据库文件加锁，锁要么SHARED_LOCK,要么RESERVED_LOCK or EXCLUSIVE_LOCK，如果调用成功，Pager.eLock状态被更新，若功能调用时，Pager.eLock被设置为UNKNOWN_LOCK。不对其进行修改，除非新的锁状态是EXCLUSIVE_LOCK
*/
static int pagerLockDb(Pager *pPager, int eLock){
  ...
  if( pPager->eLock<eLock || pPager->eLock==UNKNOWN_LOCK ){							//状态值：UNKNOWN_LOCK > EXCLUSIVE_LOCK
    rc = sqlite3OsLock(pPager->fd, eLock);			调用OS层对数据库文件加锁
...
确定atomic-write优化是否能被用于页面上。优化操作可能在如下情况下被使用：
1、OsDeviceCharacteristics()返回值，指示一个数据库页可能被原子写
2、OsSectorSize()返回值<=页大小
该优化对临时文件可用。pPager以内存数据库方式打开，调用该函数时返回错误。
如果优化不能被使用，0返回，否则返回值是包含了一个具体页面的回滚数据的日志文件的大小
*/
...
该功能被调用时,Pager必须是打开的。函数试图从日志文件中读取master journal文件名，如果成功读取，把它复制到主存中被调用者使用。
如果参数中的master journal 名称长度大于最大路径长度，将该名称当做没有被传递进函数来处理。
如果一个master journal 文件名读取出来后复制到zMaster指针指向的缓冲区中。一个终止符被加在结尾
如果没有master journal文件名被传送到,zMaster[0]被设置为0，并返回SQLITE_OK。发生错误则返回错误码
*/
...
**   100                       512
**   2000                      2048
返回扇区边界的偏移量或者直接跟随pPager->journalOff中的值，假设一个pPager->sectorSize的扇区
** 
*/
static i64 journalHdrOffset(Pager *pPager){					
  ...
  if( c ){
    offset = ((c-1)/JOURNAL_HDR_SZ(pPager) + 1) * JOURNAL_HDR_SZ(pPager);		JOURNAL_HDR_SZ==pPager->sectorSize
  }
...
函数被调用时，日志必须是打开的。如果在当前事务中，日志没有被写过（Pager.journalOff==0），该函数什么事都不做。
doTruncate非0,或者 Pager.journalSizeLimit为0，将日志文件截短为0字节。否则，将日志文件开始的28-byte header置零。
不论哪种情况，如果pager不处于no-sync mode，在写或者截短日志之后立刻同步日志文件。
如果Pager.journalSizeLimit被设置为>0的数,并且对日志文件中的归零或者截短字节大小大于这个值，则截短日志文件为Pager.journalSizeLimit 字节。该操作后，日志文件无需被同步
如果发生IO错误，不处理，只返回IO错误码，否则，返回SQLITE_OK
*/
static int zeroJournalHdr(Pager *pPager, int doTruncate){
  int rc = SQLITE_OK;                               /* Return code */
  assert( isOpen(pPager->jfd) );
  if( pPager->journalOff ){
    const i64 iLimit = pPager->journalSizeLimit;    /* Local cache of jsl */			本地的jsl缓冲
    ...
    if( doTruncate || iLimit==0 ){
      ...
      rc = sqlite3OsWrite(pPager->jfd, zeroHdr, sizeof(zeroHdr), 0);		将日志头置零
    }
    ...
    ** to sync the file following this operation.
	此时，事物已经被提交，但是写锁依然被该文件持有。如果对持有日志而言存在大小限制的配置，现在进行截断，该操作之后无需同步
    */
...
调用该函数，日志文件必须打开状态。一个日志文件头（JOURNAL_HDR_SZ bytes）被写到日志文件中at the current location
journal头的格式如下：
** The format for the journal header is as follows:									
** - 8 bytes: Magic identifying journal format.										Magic标识的日志格式
** - 4 bytes: Number of records in journal, or -1 no-sync mode is on.				日志中的记录数，-1表示开启的是no-sync mode
** - 4 bytes: Random number used for page hash.										页面哈希使用的随机数
** - 4 bytes: Initial database page count.											最初的数据库页面数
** - 4 bytes: Sector size used by the process that wrote this journal.				写该日志的进程使用的扇区大小
** - 4 bytes: Database page size.													数据库页面大小
*/
static int writeJournalHdr(Pager *pPager){
  int rc = SQLITE_OK;                 /* Return code */
  char *zHeader = pPager->pTmpSpace;  /* Temporary space used to build header */	//用于构建日志头部的临时空间
  u32 nHeader = (u32)pPager->pageSize;/* Size of buffer pointed to by zHeader */	//zHeader指向的缓冲区的大小（一个页面的大小1024）
  ...
  if( nHeader>JOURNAL_HDR_SZ(pPager) ){				
    nHeader = JOURNAL_HDR_SZ(pPager);				JOURNAL_HDR_SZ  == 扇区大小
  }
  ...
  ** reading the journal this value tells SQLite to assume that the				这个假设是危险的，就好像日志文件进行写操作时可能包含垃圾数据，一个错误发生。由两个方案可以让这个风险被忽略：
  ...
  ** take the performance hit.
  对日志尾部未使用的缓冲区进行初始化是不必要的。就算忽略掉后面的内容有效的内容都能被找到，。但是这样的初始化操作能够避免valgrind（内存调试、内存泄漏检测、性能分析工具）
  检测报告问题。宁愿损失一点性能
  */
  ...
  ** to populate the entire journal header sector.
  理论上，只需要对日志使用的28个header字节进行写入。当通过JOURNAL_HDR_SZ递增Pager.journalOff，以便下一条记录被写到后面的扇区中（在文件中存留的间隙会被OS隐含填充）
  然而发现在某些操作系统上，该模式在连续将数据写入文件时会造成速度的明显下降，即使将数据隐含写入(JOURNAL_HDR_SZ - 28)的字节块不会被使用。？？
  在sector-size大于数据库页面大小的时候，需要进行下面的循环操作。因为zHeader缓冲区仅仅只有Pager.pageSize字节大小，所有可能需要对sqlite3OsWrite()进行多次调用，来
  填充整个日志头扇区。
  */ 
...
调用该方法，日志必须被打开。日志头文件(JOURNAL_HDR_SZ bytes)从日志文件的当前位置读出，日志文件的当前位置由pPager->journalOff给出。见上述功能writeJournalHdr()。
对日志头格式的描述。如果成功读取头部， *pNRec 被设置为页面记录的数量，并且*pDbSize被设置为在事务开始前数据库的大小，以页为单位。
pPager->cksumInit被设置的值来自于对日志头的读取。这种情况下，SQLITE_OK被返回。
如果日志头文件崩溃，SQLITE_DONE被返回，并且*pNRec 和 *PDbSize未被定义。如果 JOURNAL_HDR_SZ个字节不能从日志文件中读取，将返回错误码
*/
static int readJournalHdr(
  Pager *pPager,               /* Pager object */
  int isHot,					当锁状态为shared-lock和RESERVED-lock下为热日志
  ...
  assert( isOpen(pPager->jfd) );      /* Journal file must be open. */				被打开的日志文件
    ...
    */
    if( iPageSize<512                  || iSectorSize<32
      ...
      ** the journal file here.
	  如果页面大小或者扇区大小在日志头中是无效的，在日志头被同步之前，写日志头的进程crashed。这种情况下停止读日志文件
      */
      return SQLITE_DONE;
    }
...
将提供的master journal名写到pager pPager日志文件的当前位置（对日志文件header写入）。master journal 名必须是最后一个被写入日志文件的内容。
如果pager处于full-sync模式，journal文件描述符在被写入之前进入下一个扇区的边界。这个格式是如上。master journal 名字的校验和
master-journal 页面校验和就是master journal name每个字节的和，这些字节被解释为一个8-bit有符号整数
zMaster为NULL(occurs for a single database transaction)，本调用无操作
*/
static int writeMasterJournal(Pager *pPager, const char *zMaster){
  ...
  */
  ...
  /* Write the master journal data to the end of the journal file. If					//写master journal数据到数据文件的末尾。如果发生错误则返回错误码给调用者
  ...
  */
  ...
  /* If the pager is in peristent-journal mode, then the physical 						//如果pager处于peristent-journal模式，那么物理的日志文件可能扩展到master-journal 名之后
...
*/
static void pager_reset(Pager *pPager){		备份后再清空
...
设置PagerSavepoint.pInSavepoint 中所有打开的保存点的bitvecs中的bit number pgno，
成功返回SQLITE_OK，如果内存分配失败返回SQLITE_NOMEM
*/
...
** returned.
该函数结束一个事务。一个事务通常来说，被一个COMMIT或者ROLLBACK操作结束。这个函数可能在热日志的回滚之后被调用，或者如果错误发生在打开日志文件或者写
一个数据库事务当中的最初的journal-header时。
这个函数不会在PAGER_ERROR状态被调用，如果在PAGER_NONE or PAGER_SHARED状态被调用，并且持有的锁低于RESERVED锁，它将不做任何操作。
否则，任何保存点都被释放。
如果日志文件是打开的，那么它被“finalized”. 一旦一个日志文件被finalized，它可能用它回滚一个事务。它不会被数据库连接认为是一个热日志。
一个日志具体如何被finalize取决于pager是否运行于互斥模式和当前journal-mode (Pager.journalMode value)，如下所示：
journalMode==MEMORY
日志文件描述符被关闭。这销毁了一个内存日志
journalMode==TRUNCATE
日志文件大小被截断为0字节
journalMode==PERSIST
日志文件的头28字节被置0。这是的文件中的第一个日志头变为无效，整个日志文件也是如此。一个无效的日志文件不能被回滚。
journalMode==DELETE
日志文件被关闭和删除（用sqliteOsDelete()）
如果pager在互斥模式下运行，这个finalizing日志的方法不会被使用,取而代之的如果journalMode是DELETE并且pager处于exclusive模式,这个方法在journalMode==PERSIST 模式下被使用
在日志文件被finalize之后,pager变为journalMode==READER状态.如果运行于非互斥的回滚模式下,文件上的锁被降级为共享锁.
如果没有错误发生,SQLITE_OK返回.如果错误发射给你在任何对日志文件finalize的IO操作期间或者解锁数据库期间,那么IO错误码将被返回给用户.
如果finalize日志文件的操作失败了,那么代码试图解锁数据库文件如果当前状态不在互斥模式下,解锁操作失败,那么和第一个错误发生时的第一个错误码被返回
*/
...
参数aData必须指向pPager-> pageSize字节的缓冲区中的数据。 计算并返回基于的数据页内容的校验和和pPager-> cksumInit的当前值。
这不是一个真正的校验和。 它只是总和随机初始值（pPager-> cksumInit）和每200个字节的页面数据，从字节偏移（pPager-> pageSize％200）开始。每个字节被解释为8位无符号整数。
更改用于计算此校验和的公式将导致不兼容的日志文件格式。
如果日志崩溃是由于电源故障，最可能的方案是记录的一端或另一端将被改变。
它不太可能是日志记录的两端正确，而中间崩溃。 因此，这种“校验和”方案，虽然快速和简单，但是可以捕捉可能性最大的损坏
*/
...
从日志文件中读取单个页面（如果isMainJrnl == 1）或
从子日记（如果isMainJrnl == 0）并回放该页。页面从文件中偏移量pOffset处开始。 * pOffset值增加到日志中下一页的开始。
main 回滚日志使用校验和-statement日志不使用。
如果pDone不为NULL，那么它是已经回放的页面的记录。 如果pOffset处的页面已经回放（如果相应的pDone位置1），然后跳过回放。确保返回前设置了与pOffset页对应的pDone位。
如果从（子）日志文件成功读取了页面记录并回放，然后返回SQLITE_OK。 如果同时从（子）日志文件读取记录或在写入到数据库文件时发生IO错误，然后返回IO错误代码。 如果数据
已从（子）日志文件成功读取，但似乎是损坏的，返回SQLITE_DONE。 数据被视为已损坏
两种情况（或者）：
*如果记录的页码非法（0 or PAGER_MJ_PGNO）
*如果记录从main 日志文件中被回滚并且checksum域与记录内容不匹配
在保存点回滚期间以上两种情况可能都不发生。
如果是一个savepoint回滚，主存可能由函数被动态分配。这种情况下分配失败，返回SQLITE_NOMEM
*/
static int pager_playback_one_page(
  Pager *pPager,                /* The pager being played back */
  i64 *pOffset,                 /* Offset of record to playback */				要回滚的记录的偏移量
  Bitvec *pDone,                /* Bitvec of pages already played back */		已经回滚的Bitvec中的页面
  int isMainJrnl,               /* 1 -> main journal. 0 -> sub-journal. */		
  int isSavepnt                 /* True for a savepoint rollback */				true：保存点回滚：用户级别事务的回滚操作（语句级别）
){
  int rc;
  PgHdr *pPg;                   /* An existing page in the cache */				cache中一个存在的页面
  ...
  ** only reads from the main journal, not the sub-journal.
  回滚情况：
  1、状态大于PAGER_WRITER_CACHEMOD模式(事务或者保存点回滚已经在调用者请求时完成)【操作中断导致回滚】
  2、是一个热日志回滚，pager于OPEN状态并持有一个互斥锁。【崩溃恢复请求回滚】
  */
  ...
  */
  ...
  rc = read32bits(jfd, *pOffset, &pgno);				//pOffset：日志记录起始位置（不含头）
  if( rc!=SQLITE_OK ) return rc;
  rc = sqlite3OsRead(jfd, (u8*)aData, pPager->pageSize, (*pOffset)+4);
  if( rc!=SQLITE_OK ) return rc;
  *pOffset += pPager->pageSize + 4 + isMainJrnl*4;												+ isMainJrnl*4 ？？【这四位是校验和存储的位置】

  /* Sanity checking on the page.  This is more important that I originally
  ** thought.  If a power failure occurs while the journal is being written,
  ** it could cause invalid data to be written into the journal.  We need to
  ** detect this invalid data (with high probability) and ignore it.
  页面的完整性检查。如果日志被写期间发生电源故障，无效数据会被写入日志，我们需要检测无效数据（可能性很高）并忽视它
  */
...
如果Pager处于CACHEMOD状态，则必须有一个页面副本。在这种情况下，只需更新pager缓存，而不是数据库文件。在这种情况下，页面被标记为脏。上述规则的例外：如果数据库处于非同步模式并且页面在增量清空期间移动，则页面可以不在Pager缓存中。
之后：如果发生malloc（）或IO错误在Movepage（）调用期间，页面可能不在缓存中或者。所以上面描述的条件不是assert（）能检查的。如果在WRITER_DBMOD，WRITER_FINISHED或OPEN状态，那么我们更新pager缓存（如果存在）和主文件。然后标记该页面
不是脏页面。因为这段代码只在PAGER_OPEN状态下执行热日志回滚，以保证pager处于OPEN状态时页面缓存为空。
故障单＃1171：语句日志包含的页面内容可能不同于事务开始时的页面内容。这在页面在语句开始之前更改时发生然后在语句内再次更改。当回滚这样一个语句日志时我们不能将其写入原始数据库，除非我们确切的知道以确保原始页面内容同步到主回滚中。
否则，断电可能会把修改的数据留在数据库文件中，但是日志文件中却没有对应的可以将数据库恢复到其原始状态的条目。必须有两个条件在写入数据库文件之前达到：（1）数据库必须是锁定。
（2）我们知道原始页面内容是完全同步的在主日志中，因为页面不在缓存中或其他该页面被标记为needSync == 0。

2008-04-14：当试图清除一个损坏的数据库文件，它可能导致数据库上尚不存在的语句失败。如果数据库文件从未打开，请勿尝试写入。
  */
  if( pagerUseWal(pPager) ){
    pPg = 0;
  }else{
...
如果这是保存点回滚、数据没有写入数据库、页面不在内存中，则存在潜在问题。 当页面下一次被b-tree层提取时，它将从数据库文件中进行读取，这可能或可能不是当前。

有几种不同情况可能发生。 所有都很不清晰。 当在同步模式下运行时，这仅发生在如果事务开始时，如果当页面在空闲列表上，则填充，然后使用sqlite3PagerMovepage（）移动。
 
解决方案是将一个内存页添加到包含的缓存中数据只是从子日志中读取。 将页面标记为脏并且如果pager需要日志同步，则将该页面标记为在写入之前需要进行日志同步。
    */
    assert( isSavepnt );
...
重放日志并恢复数据库文件。
日志文件格式如下：
（1）8字节前缀。 aJournalMagic []的副本。
（2）4字节的big-endian整数，它是日志中有效页记录的数量。 如果此值为0xffffffff，则从日志大小计算页面记录数。
（3）4字节的big-endian整数，它是完整性校验和初始值。
（4）4字节整数，即要截断的页面数数据库到回滚期间。
（5）4字节的大端整数，即扇区大小。header是这个许多字节大小。
（6）4字节的大端整数，即页大小。
（7）零填充到下一扇区大小。
（8）零个或多个页面实例，每个实例如下：
       + 4字节页码。
       + pPager-> pageSize字节的数据。
       + 4字节校验和
当我们谈到日志头时，我们是指上面的前7个项目。日志中的每个条目都是第8个项目的实例。
从第二个项目符号“nRec”调用该值。 nRec是个数
日志中的有效页条目。 在大多数情况下，您可以从日志文件的大小计算nRec的值。 但如果电源失效发生在日志正在写的过程中，它可能是
这种情况：日志文件的大小已经增加，但是额外的条目还没有安全到磁盘。 在这种情况下，从文件大小计算的nRec的值将会过大。 对于那个原因，我们总是使用头中的nRec值。

如果nRec值为0xffffffff，则意味着应该从文件大小计算nRec。 该值在用户选择时使用日志的no-sync选项。在这种情况下电源故障可能导致损坏。但对于像临时表（电源恢复时
删除）我们不在乎。如果作为日志文件打开的文件不是格式正确的日志文件，则滚动到第一个损坏页面之前的所有页面（如果日志头损坏，则没有页）。 删除日志文件并返回SQLITE_OK，就好像没有遇到损坏。
如果发生I / O或malloc（）错误，则不会删除日志文件并返回错误代码。
*/
...
<<<<<<< HEAD
  计算日志文件中有多少条记录。如果日志文件是空的，则返回
=======
  计算出日志文件中有多少日志记录。如果日志是空的，则中断操作。
>>>>>>> 80e276d5822064d856d202a8f52da72f3d10b36b
  */
  assert( isOpen(pPager->jfd) );
  ...
  ** for pageSize.
<<<<<<< HEAD
  从回滚日志中读取master日志名。如果指定了一个master日志名，但是日志文件未在磁盘上，表明当前日志不是热日志并且无需回滚。
=======
...
>>>>>>> 80e276d5822064d856d202a8f52da72f3d10b36b
  */
  ...
  rc = readMasterJournal(pPager->jfd, zMaster, pPager->pVfs->mxPathname+1);		读master日志名
  if( rc==SQLITE_OK && zMaster[0] ){											日志名存在
    rc = sqlite3OsAccess(pVfs, zMaster, SQLITE_ACCESS_EXISTS, &res);	访问该日志
  }
  zMaster = 0;
  if( rc!=SQLITE_OK || !res ){		
    goto end_playback;			结束重放
  }
  pPager->journalOff = 0;			
  ...
  ** occurs. 
<<<<<<< HEAD
  当readJournalHdr()或者 pager_playback_one_page()返回 SQLITE_DONE或发生IO错误，则循环中断
=======
  满足以下条件之一，循环终止：
...
<<<<<<< HEAD
	读取下一个日志头。如果剩余字节数小于日志文件头的大小，则表明没有其他的需要被回滚。
=======
...
<<<<<<< HEAD
	如果nRec为0xffffffff，表明进程以异步方式运行。表明日志剩余的部分由多个页面构成，已没有更多日志头，并根据文件大小计算记录数
=======
...
>>>>>>> 80e276d5822064d856d202a8f52da72f3d10b36b
    ...
    ** database file back to its original size.
	如果是从journal读取的第一个头，阶段数据库文件为其原始大小
    */
...
end_playback:
  ...
  ** assertion that the transaction counter was modified.
  跟随一个回滚，数据库文件应该回到事务开始时的初始状态，调用SQLITE_FCNTL_DB_UNCHANGED文件控制方法关闭对事务计数器修改检查
  */
#ifdef SQLITE_DEBUG
  ...
  ** in case this has happened, clear the changeCountDone flag now.
  如果因为一次IO或者内存分配失败而导致自动回滚(change-counter更新之后，事务提交之前)，change-counter 的修改会被恢复。
  如果这发生在互斥模式下，事务后续的执行将不会再更新 change-counter。这可能导致缓存内容不一致问题的发生功能。所以，
  为了防止这样的情况发生，清除changeCountDone标记
  */
  pPager->changeCountDone = pPager->tempFile;

  if( rc==SQLITE_OK ){
    zMaster = pPager->pTmpSpace;
    rc = readMasterJournal(pPager->jfd, zMaster, pPager->pVfs->mxPathname+1);读取Master日志名
    testcase( rc!=SQLITE_OK );
  }
  if( rc==SQLITE_OK
   && (pPager->eState>=PAGER_WRITER_DBMOD || pPager->eState==PAGER_OPEN)
  ){
    rc = sqlite3PagerSync(pPager);			将修改同步到数据库文件
  }
  if( rc==SQLITE_OK ){
    rc = pager_end_transaction(pPager, zMaster[0]!='\0');			结束当前事务
    testcase( rc!=SQLITE_OK );
  }
  if( rc==SQLITE_OK && zMaster[0] && res ){
    /* If there was a master journal and this routine will return success,
    ** see if it is possible to delete the master journal.
	如果存在master日志并且返回成功，删除master日志。
    */
...
从数据库文件读取页面内容到pPg，并写到pPg->pData中。该功能调用前，需要持有数据库文件上的共享锁或者更大的锁。
如果页面1被读取，那么Pager.dbFileVers[]的值被设置为从数据库文件中读取到的值。根据操作结果返回结果码
*/
static int readDbPage(PgHdr *pPg){
  ...
  if( pagerUseWal(pPager) ){
    /* Try to pull the page from the write-ahead log. */	尝试从预写的日志记录中拉取页面
...
更新在header中在偏移量24和92出的change-counter和96处的sqlite版本号。
该操作为无条件的更新操作。见pager_incr_changecounter()，该函数只在实际需要的时候更新change-counter，更新与否根据pPager-changeCountDone状态变量来确定
...
当pager状态从PAGER_OPEN转换为PAGER_READER时，该函数作为转换过程中的部分内容用来确定数据库文件的页面数的大小（假设当前页面大小存储在Pager.pageSize中）。
操作成功将页面数量设置到第二个参数中，否则保持其内容不变
*/
...
播放保存点pSavepoint。 或者，如果pSavepoint == NULL，则播放整个master日志文件。 当在保存点事务的SAVEPOINT上调用ROLLBACK TO命令，发生pSavepoint == NULL这种情况。
当pSavepoint不为NULL（意味着被回滚的是非事务保存点），则回滚包括最多三个阶段，按指定的顺序执行：
   *从主日志的PagerSavepoint.iOffset偏移量开始回放到PagerSavepoint.iHdrOffset或者到主日志文件
     PagerSavepoint.iHdrOffset，或者到主日志的结尾
     文件，如果PagerSavepoint.iHdrOffset为零。

   *如果PagerSavepoint.iHdrOffset不为零，则播放页面
     从紧随其后的日志标题开始
     PagerSavepoint.iHdrOffset到主日志文件的结尾。

   *然后从子日志文件开始播放页面
     与PagerSavepoint.iSubRec并继续到结束
     日志文件。
...
static int hasHotJournal(Pager *pPager, int *pExists){
  ...
  *pExists = 0;
  if( !jrnlOpen ){				日志文件未打开
    rc = sqlite3OsAccess(pVfs, pPager->zJournal, SQLITE_ACCESS_EXISTS, &exists);		访问日志文件
  }
  if( rc==SQLITE_OK && exists ){			日志文件存在？
    int locked = 0;             /* True if some process holds a RESERVED lock */true：某个进程持有RESERVED锁
    ...
    ** be dealt with by the playback routine.  Ticket #3883.
	其他进程可能已经持有RESERVED锁，并且已经有一个日志文件被打开，但是在我们调用 sqlite3OsCheckReservedLock()之前删除了该日志文件并且移除了锁。
	这种情况下，该函数可能会认为当前存在一个热日志（实际上没有）。
    */
    ...
    if( rc==SQLITE_OK && !locked ){
      ...
      ** [H33020].
	  检查数据库文件大小。如果为没有页面，则删除日志文件。删除RESERVED下的废弃的日志文件以避免系统紊乱。
      */
      rc = pagerPagecount(pPager, &nPage);
      if( rc==SQLITE_OK ){
        if( nPage==0 ){
          ...
          ** it can be ignored.
		  日志文件存在，并且其他连接不持有reserved或以上的锁。则检查日志文件头至少存在一个非0的字节。如有，我们认为日志为热日志；否则，日志被忽略。
          */
          if( !jrnlOpen ){
          ...
          if( rc==SQLITE_OK ){
            ...
            ** worry so much with race conditions.
			如果无法打开回滚日志文件来查看是否有一个0头，可能是由于I/O错误，或者因为上述描述的系统紊乱。无论什么情况，我们都假设日志是热日志。这可能是一个错误假设。
			但是如果是这样的话，原子日志回访和恢复机制将会在EXCLUSIVE锁下处理它，我们无需担心当前的系统紊乱
            */
            *pExists = 1;
            rc = SQLITE_OK;
          }
...
用来获取一个数据库文件上的共享锁。在该函数成功被调用之后，对函数sqlite3PagerAcquire()的调用才是合法的。如果已经持有共享锁，无操作。

该函数执行以下操作：
1）如果pager当前为PAGER_OPEN状态（当前数据库文件上没有锁），则试图获取一个共享锁。获取共享锁之后，进行热日志的检查，如果有的话进行回滚。进行回滚之后，通过检查数据库文件头的‘change-counter'域确定缓存中内容的有效性，如果无效
则丢弃它。
2）如果pager运行在互斥模式，并且处于错误状态。那么尝试通过丢弃缓存中的内容及回滚任何打开的日志文件来清除错误状态。
*/
int sqlite3PagerSharedLock(Pager *pPager){
  ...
  */
  assert( sqlite3PcacheRefCount(pPager->pPCache)==0 );			//当前pager没有页面被引用
  assert( assert_pager_state(pPager) );
  assert( pPager->eState==PAGER_OPEN || pPager->eState==PAGER_READER );		状态=PAGER_OPEN||PAGER_READER
  if( NEVER(MEMDB && pPager->errCode) ){ return pPager->errCode; }

  if( !pagerUseWal(pPager) && pPager->eState==PAGER_OPEN ){		未使用wal并且pager 状态为PAGER_READER
    int bHotJournal = 1;          /* True if there exists a hot journal-file */

    assert( !MEMDB );

    rc = pager_wait_on_lock(pPager, SHARED_LOCK);				获取共享锁
    if( rc!=SQLITE_OK ){
      assert( pPager->eLock==NO_LOCK || pPager->eLock==UNKNOWN_LOCK ); 获取失败，要么NO_LOCK 要么UNKNOWN_LOCK
      goto failed;
    }
...
<<<<<<< HEAD
	如果存在日志文件，并且数据库文件上不持有RESERVED锁，要么回滚，要么删除
=======
...
>>>>>>> 80e276d5822064d856d202a8f52da72f3d10b36b
    */
    ...
    if( bHotJournal ){		如果存在热日志
      ...
      */
      rc = pagerLockDb(pPager, EXCLUSIVE_LOCK);						获取EXCLUSIVE锁
      if( rc!=SQLITE_OK ){
        goto failed;
      }
      ...
      */
      if( !isOpen(pPager->jfd) ){			如果日志未打开
        sqlite3_vfs * const pVfs = pPager->pVfs;
        int bExists;              /* True if journal file exists */
        rc = sqlite3OsAccess(					访问日志
            pVfs, pPager->zJournal, SQLITE_ACCESS_EXISTS, &bExists);
        if( rc==SQLITE_OK && bExists ){		访问成功&&日志存在
          int fout = 0;
          int f = SQLITE_OPEN_READWRITE|SQLITE_OPEN_MAIN_JOURNAL;		
          assert( !pPager->tempFile );
          rc = sqlite3OsOpen(pVfs, pPager->zJournal, pPager->jfd, f, &fout);		打开日志
          assert( rc!=SQLITE_OK || isOpen(pPager->jfd) );
          if( rc==SQLITE_OK && fout&SQLITE_OPEN_READONLY ){		
            rc = SQLITE_CANTOPEN_BKPT;
            sqlite3OsClose(pPager->jfd);		关闭日志
          }
        }
      }
      ...
      */
      if( isOpen(pPager->jfd) ){			打开成功
        assert( rc==SQLITE_OK );
        rc = pagerSyncHotJournal(pPager);		同步热日志
        if( rc==SQLITE_OK ){
          rc = pager_playback(pPager, 1);		对日志内容进行回放
          pPager->eState = PAGER_OPEN;		状态=PAGER_OPEN
        }
      }else if( !pPager->exclusiveMode ){	pager不处于互斥模式
        pagerUnlockDb(pPager, SHARED_LOCK);	解锁 为SHARED_LOCK
      }
...
<<<<<<< HEAD
    if( !pPager->tempFile 
     && (pPager->pBackup || sqlite3PcachePagecount(pPager->pPCache)>0) 			非临时文件 && （存在备份||存在已换存的页）
=======
...
>>>>>>> 80e276d5822064d856d202a8f52da72f3d10b36b
      ...
      */
      ...
      rc = pagerPagecount(pPager, &nPage);			获取页数
      if( rc ) goto failed;

      if( nPage>0 ){		页数>0
        IOTRACE(("CKVERS %p %d\n", pPager, sizeof(dbFileVers)));
        rc = sqlite3OsRead(pPager->fd, &dbFileVers, sizeof(dbFileVers), 24);	读取dbFileVers
        if( rc!=SQLITE_OK ){
          goto failed;
        }
      }else{
  ...
  if( pagerUseWal(pPager) ){			使用了WAL
    assert( rc==SQLITE_OK );
<<<<<<< HEAD
    rc = pagerBeginReadTransaction(pPager);		开启读事物
  }

  if( pPager->eState==PAGER_OPEN && rc==SQLITE_OK ){	状态为PAGER_OPEN
    rc = pagerPagecount(pPager, &pPager->dbSize);		页数设置为数据库文件包含的页数
  }
...
** or journal files，
在 pPager中获取页号pgno的引用（页引用类型为DbPage *）。若成功获取引用，将其复制到* ppPage，返回SQLITE_OK。如果请求的页面已经在缓存中，则返回。 否则，将分配一个新的页面对象并使用从数据库文件读取的数据进行填充。
在一些情况下，pcache模块可以选择不分配新的页面对象，并且可以重用没有未完成引用的现有对象。

当首次将页面加载到内存中时，附加到页面的额外数据总是初始化为零。 如果当这个函数被调用的时候请求的页是已经在缓存中，额外的数据保留为上次使用页面对象时的状态。如果数据库镜像小于请求的页面，
或者如果传递非零值作为noContent参数，并且请求的页面尚未存储在高速缓存中，则不会发生实际的磁盘读取。 在这种情况下，页面的内存镜像被初始化为全零。
如果noContent为true，则意味着我们不关心页面的内容。 这发生在两种的情况下：
a）从数据库读取空闲列表叶子页面时
b）当保存点正在回滚，并且我们需要将一个新页面加载到缓存中以填充从保存点日志读取的数据。
...
在每个写事务开始时调用此函数。当调用此例程时必须已在数据库上有RESERVED或EXCLUSIVE锁文件。打开pPager的日志文件，并在开始位置写一个日志头。 如果有活动的保存点，还要打开子日志。 
此函数仅在日志文件被打开来写入事务的回滚日志时调用。 不被调用来打开热日志文件用以回滚的情况。如果日志文件已经打开（因为它可能在独占模式下）那么这个函数只是写一个日志头到开始的已打开文件。如果通过此函数打开日志文件，
Pager.pInJournal的bitvec结构被分配。如果一切都成功，返回SQLITE_OK。 否则，返回SQLITE_NOMEM如果尝试分配Pager.pInJournal失败，或如果打开或写入日志文件失败，则会显示IO错误代码。
*/
  ...
  if( ALWAYS(pPager->eState==PAGER_READER) ){
    ...
    if( pagerUseWal(pPager) ){
      ...
      ** exclusive lock on the database is not already held, obtain it now.
	  如果pager配置为locking_mode=exclusive时，如果没有持有，获取该锁。
      */
...
将数据页标记为可写的。根据需要将页面写入主日志或子日志中，Pager.pInJournal bitvec相应的位和PagerSavepoint.pInSavepoint bitvecs被打开的相应位被设置
*/
...
int sqlite3PagerSync(Pager *pPager){
  int rc = SQLITE_OK;
  if( !pPager->noSync ){		noSync标记为0
    assert( !MEMDB );
    rc = sqlite3OsSync(pPager->fd, pPager->syncFlags);		调用OsSync
  }else if( isOpen(pPager->fd) ){	如果数据库文件已打开
    assert( !MEMDB );		
    rc = sqlite3OsFileControl(pPager->fd, SQLITE_FCNTL_SYNC_OMITTED, 0);	同步忽略
...
该函数被调用时，数据库文件被完全更新，以反映当前事务所做的更改并同步到磁盘。日志文件依然存在在文件系统中。如果在此时发生故障，它最终会使用热日志将当前事务回滚。此函数通过删除、截断活儿部分归零日志文件来结束记录日志，
以使它不能被用于热日志回滚。一旦该操作完成，事务的提交不可撤销。
根据操作结果返回对应操作码
*/
int sqlite3PagerCommitPhaseTwo(Pager *pPager){
  ...
  ** called, just return the same error code without doing anything. */如果之前发生了一个错误，该函数应该不会被调用。
  if( NEVER(pPager->errCode) ) return pPager->errCode;
...
<<<<<<< HEAD
  一个优化。在当前事务中，数据库实际上未被修改，pager运行在exclusive-mode并且使用的是persistent journals，那么该函数无操作
=======
...
如果一个写事务被打开，那么在事务内的所有改变被还原，并且当前写事务被关闭。如果成功，pager返回PAGER_READER状态，或者PAGER_ERROR状态，如果发生错误。如果在调用该函数时pager已经处于PAGER_ERROR状态，
它立即返回Pager.errCode。在这种情况下不执行任何工作。否则，在回滚模式下，此函数执行两个功能：
  1）它回滚日志文件，恢复所有数据库文件和内存缓存页面为事务打开是的状态，以及
  2）它完成日志文件，以便它不再被用于热日志回滚。
日志文件（任务2）的完成只有在回滚成功时执行。
在WAL模式下，当前事务中所有包含数据修改的缓存条目要么从缓存中移出，要么通过从数据库或WAL文件重新读取数据，恢复到 pre-transaction状态，然后关闭WAL事务。
*/
...
#ifdef SQLITE_TEST
/*
** This routine is used for testing and analysis only.被用来测试和分析
*/
...
** returning.
参数eState取值：SQLITE_DBSTATUS_CACHE_HIT 或 SQLITE_DBSTATUS_CACHE_MISS。
*/
...
** returned. Otherwise, SQLITE_OK.
	...
	检查是否有至少nSavepoint打开。如果打开的savepoint少于nSavepoints个，然后打开一个或多个保存点以弥补差异。如果相等，无操作
*/
...
该函数被调用来对savepoint进行回滚或者释放（提交）。将要释放或者回滚的savepoint无需是最新创建的。

参数op取值：SAVEPOINT_ROLLBACK或者SAVEPOINT_RELEASE。如果是SAVEPOINT_RELEASE，用索引iSavepoint释放并销毁savepoint。如果是SAVEPOINT_ROLLBACK，回滚从savepoint创建声明开始后发生的所有改变。

iSavepoint标识要回滚或释放的savepoint。0标识：操作与最外面的保存点（第一次创建的）。值(Pager.nSavepoint-1)：操作最近创建的savepoint。如果iSavepoint大于(Pager.nSavepoint-1)，那么该函数无操作。

如果传入的值是负数：回滚当前事务。这和sqlite3PagerRollback()不同，因为该函数不终止事务或者解锁数据库，它仅仅只是将数据库内容恢复到初始状态。

无论哪种情况，所有索引大于iSavepoint的savepoints被销毁。如果是释放操作：iSavepoint标识的savepoint也被销毁。
根据操作结果返回对应的结果码。
*/ 
...
/*
** Return the full pathname of the database file.		返回数据库文件的完成路径名。
**
...
不能有对以前位于pgno的页面引用（我们称为pPgOld），尽管该页面被允许在缓存中。如果先前位于pgno的页面尚未存在在回滚日志中，它不是由这个函数放在那里。

对页面pPg的引用仍然有效。pager负责更新任何与pPg相关联的元数据（即存储在随着page一起分配的nExtra字节中的数据）。

调用此例程时，事务必须处于活动状态。它曾经是要求语句事务不活动，但此限制已被删除（当语句事务处于活动状态时，CREATE INDEX需要移动一个页面）。

如果第四个参数isCommit是非零，则此页面将在提交事务之前作为数据库重新组织的一部分进行移动。在这种情况下，它保证数据库页pPg指在此事务中不会再次写入。

如果出现错误，此函数可能返回SQLITE_NOMEM或IO错误代码发生。否则，它返回SQLITE_OK。
*/
...
** locking-mode.
获取/设置对应pager的locking-mode。参数eMode必须是以下值之一：PAGER_LOCKINGMODE_QUERY, PAGER_LOCKINGMODE_NORMAL, PAGER_LOCKINGMODE_EXCLUSIVE.如果参数不是_QUERY，那么locking-mode被设置为指定的值。

返回值： PAGER_LOCKINGMODE_NORMAL 或者 PAGER_LOCKINGMODE_NORMAL ，指示当前的锁模式。
*/
...
设置journal-mode。参数eMode取值必须为以下值之一：
**    PAGER_JOURNALMODE_DELETE
**    PAGER_JOURNALMODE_TRUNCATE
**    PAGER_JOURNALMODE_PERSIST
**    PAGER_JOURNALMODE_OFF
...
**    PAGER_JOURNALMODE_WAL
对值的修改在以下情况下不被允许：
内存数据库：journal-mode只能是_OFF或者_MEMORY.
临时数据库：journal-mode不能为_WAL 
*/
...
** is unmodified.
TRUE：表示pager当前的状态，支持journalmode的修改。（仅当数据库未被修改的时候）
*/
...
获取/设置用于持久日志文件的size-limit
size limit 被设置为-1：表示没有限制。小于-1则无操作
*/
...
返回变量pPager->pBackup。backup.c中的备份模块维护该变量的内容。此模块将其不透明地用作sqlite3BackupRestart（）和
sqlite3BackupUpdate（）的参数。
*/
...
用户调用"PRAGMA wal_checkpoint", "PRAGMA wal_blocking_checkpoint" or calls the sqlite3_wal_checkpoint() 或者 wal_blocking_checkpoint() API 函数时，该功能被调用
参数eMode：取值为以下值之一：SQLITE_CHECKPOINT_PASSIVE, FULL, RESTART.
*/
...
试图获取数据库文件上的exclusive锁，如果获得的是PENDING锁，马上释放
*/
...
调用sqlite3WalOpen()打开WAL句柄。如果函数调用时pager为exclusive-locking模式，获取exclisive锁，并使用堆内存去存储wal-index。否则使用普通的shared-memory
*/
...
调用者必须持有数据库上的一个SHARED锁来调用该函数。

如果pager作为第一个参数传入，用以打开一个真实的数据库文件（非临时或内部才能数据库），并且WAL文件还未打开，尝试将其打开。如果成功，返回SQLITE_OK。如果发生错误或者VFS不支持xShmXXX()方法，返回一个错误码。
*pbOpen在任何情况下都不修改。

如果pager在一个临时文件或者内存数据库上打开，或者WAL文件已经打开，将*pbOpen设置为1并返回SQLITE_OK
*/
...
该函数在把WAL模式转换为rollback模式之前被调用来关闭日志文件连接。

关闭日志记录文件之前，该函数尝试获取一个EXCLUSIVE锁。如果不能获取，返回一个错误(SQLITE_BUSY) 并且日志记录连接不被关闭。如果成功，返回前EXCLUSIVE锁不释放。
*/
...
** is empty, return 0.
调用该功能时，必须持有数据库文件上的read-lock。如果pager是WAL模式并且，WAL文件当前包含一个或者多个frames，返回页镜像的字节大小。否则，如果这不是一个WAL数据库或者WAL文件是空的，返回0
*/
...
<<<<<<< HEAD
当将页面内容写到日志记录文件时，该函数被wal模块调用。
此函数返回一个指向包含加密页面内容的缓冲区的指针。 如果malloc失败，此函数可能返回NULL。
=======
返回一个已加密内容
>>>>>>> a53ab319d7b889d859c335709605578b1b661ee1
*/
